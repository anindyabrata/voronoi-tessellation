...
namespace fsl3d{
	...
	public:
		...
		Event_type(int index, int afid, vertex_type vert, vertex_type sitev){
			...
			y = EPS + vert.y() + sqrt((long double)CGAL::squared_distance(vert, sitev));
		}
		int ind, fid;
		vertex_type v, sv;
		scalar_type y;
	...
	Voronoi generate_fortunes_voronoi(std::vector<vertex_type> site_list){
		...
		std::set<int> beachline; // Sites whose cell is still not complete (not squeezed yet)
		std::map<unsigned int, std::vector<int>> pending_faces; // Faces that are not confirmed; Does not contain impossible faces
		...
		while(!q.empty()){
			...
			if(ev.is_site_event){
				...
				for(auto bs: pending_faces[site]) if(bs >= 0) pending_faces[bs].push_back(site);
				// Bisect beachline faces by site
				for(auto beach_site: beachline){
					auto &pasites = pending_faces[beach_site];
					std::vector<int> npasites;
					for(auto pasite: pasites){
						if(pasite < beach_site){
							...
							auto it = std::find(pending_faces[pasite].begin(), pending_faces[pasite].end(), beach_site);
							if(it != pending_faces[pasite].end()) npasites.push_back(pasite);
						}
					}
